...
void sortStudentsByName(FILE* file, int num_students) {
    Student students[num_students];
    fread(students, sizeof(Student), num_students, file);
...
void sortStudentsByCode(FILE* file, int num_students) {
    Student students[num_students];
    fread(students, sizeof(Student), num_students, file);
...
void printStudents(FILE* file, int num_students) {
    Student student;
...
void registerStudent() {
    ...
    while (num_students < MAX_STUDENTS) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            student.final_grade += student.grades[i];
        }
...
void listStudents() {
    ...
    do {
        ...
        switch(option) {
            case 1:
                printStudents(file,option);

                break;
            case 2:
                sortStudentsByName(file,option);
                printStudents(file,option);
                break;
            case 3:
                printStudents(file,option);
